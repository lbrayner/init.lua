local M = {}

function M.clip(text)
  if text then
    text = tostring(text)
    vim.fn.setreg('"', text)
  end
  vim.fn.setreg("+", vim.fn.getreg('"'))
  vim.notify(vim.fn.getreg("+"))
end

local function first_level(result)
  if not result then
    vim.notify("Unable to determine the first level. Check the working directory.")
    return
  end
  M.clip(result)
end

vim.api.nvim_create_user_command("Clip", function(command)
  M.clip(command.args)
end, { nargs = "?" })
vim.api.nvim_create_user_command("Cwd", function()
  M.clip(require("lbrayner.path").get_cwd())
end, { nargs = 0 })
vim.api.nvim_create_user_command("Directory", function()
  M.clip(require("lbrayner.path").get_directory())
end, { nargs = 0 })
vim.api.nvim_create_user_command("FirstLevel", function()
  first_level(require("lbrayner.path").get_first_level())
end, { nargs = 0 })
vim.api.nvim_create_user_command("FirstLevelName", function()
  first_level(require("lbrayner.path").get_first_level_name())
end, { nargs = 0 })
vim.api.nvim_create_user_command("FolderName", function()
  M.clip(require("lbrayner.path").get_folder_name())
end, { nargs = 0 })
vim.api.nvim_create_user_command("FullPath", function()
  M.clip(require("lbrayner.path").get_full_path())
end, { nargs = 0 })
vim.api.nvim_create_user_command("Name", function()
  M.clip(require("lbrayner.path").get_name())
end, { nargs = 0 })
vim.api.nvim_create_user_command("Path", function()
  M.clip(require("lbrayner.path").get_path())
end, { nargs = 0 })
vim.api.nvim_create_user_command("RelativeDirectory", function()
  M.clip(require("lbrayner.path").get_relative_directory())
end, { nargs = 0 })
vim.api.nvim_create_user_command("WorkingDirectoryName", function()
  M.clip(require("lbrayner.path").get_working_directory_name())
end, { nargs = 0 })

return M
