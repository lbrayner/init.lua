...
vim.api.nvim_create_autocmd("CompleteDone", {
  ...
  callback = function(args)
    ...
    if completion_item.additionalTextEdits then
      local delayed_completion = vim.v.completed_item.user_data.nvim.lsp.delayed_completion
      if delayed_completion then
        local snippet
        local completion_word = delayed_completion.completion_word
        if completion_item.textEdit then
          ...
          snippet = completion_item.insertText
